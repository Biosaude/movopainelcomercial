...
export function BaseManagement({
  ...
  const downloadTemplate = () => {
    ...
    for (let i = 0; i < maxLen; i++) {
      ...
      const m = metaRows[i];
      aoa.push([
        f?.gr ?? "", f?.rep ?? "", f?.assessor ?? "", f?.uf ?? "", f?.ufCliente ?? "", f?.ufHospital ?? "",
        f?.cliente ?? "", f?.medico ?? "", f?.marca ?? "", f?.topico ?? "", f?.tipo ?? "",
        f?.mes ?? "", f?.periodo ?? "", f?.valor ?? "",
        "",
        m?.gr ?? "", m?.rep ?? "", m?.uf ?? "", m?.ufHospital ?? "", m?.marca ?? "", m?.topico ?? "", m?.tipo ?? "", m?.periodo ?? "", m?.meta ?? "", m?.mes ?? "", m?.metaFinanceira ?? "",
      ]);
    }
